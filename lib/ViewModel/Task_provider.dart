  ...
  /// methods
   ...
   //Constructor
    ...
    Future<void> AddTask(Task task) async {
      Database DB = await DBinst.getDB();
        ...
        DBinst.Insert(DB, task.tomap());
          Future<List<Task>> newTasks = DBinst.select(DB,0); // in place of bit 0 because the new task added ans shown
    ...
    Future<void> toggleTask(int index) async {
      ...
      final String id = _tasks[index].taskId!; // store the id for counter DB delay
      int Cheker = _tasks[index].isCompleted ? 1 : 0; // change boolean to 0 and 1 to update DB
   ...
   Future<void> RemoveDBTask(int index) async {
    ...
    Future<void> HistorytoggleTask(int index) async {
      ...
      final String id = _Historytasks[index].taskId!; // store the id for counter DB delay
